...
RSpec.describe Powerpoint::Slide::TextPictureSplit do
  ...
  describe "#save" do
    ...
    it "calls save_rel_xml with correct arguments" do
      expect(slide).to have_received(:save_rel_xml).with("text_picture_split_rel.xml.erb", extract_path, index)
    end

    it "calls save_slide_xml with correct arguments" do
      expect(slide).to have_received(:save_slide_xml).with("text_picture_split_slide.xml.erb", extract_path, index)
    end
  end
  ...
  describe "#default_coords" do
    context "when dimensions are available" do
      ...
      it "calculates coords based on image dimensions" do
        start_x       = slide.send(:pixle_to_pt, 360)
        default_width = slide.send(:pixle_to_pt, 300)
